...
def cv_status(project: ProjectObject) -> str:
    return project.maturity.replace("_", " ")


def atop_summary(project: ProjectObject) -> str:
    verb = "Developed" if project.maturity in {"early_stage", "active_scaffold"} else "Advanced"
    if project.cv_section == "public_health":
        verb = "Organized"
    if project.cv_section == "archive_history":
        verb = "Structured"
    if project.cv_section == "intake":
        verb = "Scoped"
    return f"{verb} {project.public_summary[0].lower() + project.public_summary[1:]} to support {project.near_term_needs[0].lower() + project.near_term_needs[1:]}"


def write_current_projects(projects: list[ProjectObject]) -> None:
    lines = ["% Auto-generated by scripts/stem_cv_curator.py.", "% Edit data/pipeline_repos.json or upstream README/status surfaces rather than hand-editing this file.", "\\section*{Research Experience}"]
    for section, entries in sectioned(projects).items():
        if not entries:
            continue
        lines.append(f"\\subsection*{{{tex_escape(SECTION_LABELS[section])}}}")
        for p in entries:
            lines.append(f"\\cvrole{{{tex_escape(p.title)}}}{{{tex_escape(cv_status(p))}}}")
            lines.append(f"\\cvplace{{{tex_escape(p.repository.repo_name)}}}")
            lines.append("\\begin{itemize}")
            lines.append(f"    \\item {tex_escape(atop_summary(p))}")
